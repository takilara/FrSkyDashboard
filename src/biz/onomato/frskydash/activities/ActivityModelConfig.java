...
public class ActivityModelConfig extends ActivityBase implements
	...
	public void onClick(View v) {
		...
		case R.id.modConf_btnAddChannel:
			...
			// c.setId(-1);
			// _model.addChannel(c);
			Intent editChannelIntent = new Intent(getApplicationContext(),
		...
		case R.id.modConf_btnFrSkyAlarms:
			...
			i.putExtra("modelId", (int) _model.getId()); // Should edit existing
															// model
	...
	private void populateChannelList() {
		...
		for (Channel c : _model.getChannels().values()) {
			...
			tvDesc.setLayoutParams(new LinearLayout.LayoutParams(0,
			...
			int height = (int) TypedValue.applyDimension(
					TypedValue.COMPLEX_UNIT_DIP, 50, getResources()
							.getDisplayMetrics());
			btnDelete.setLayoutParams(new LinearLayout.LayoutParams(height,
					height));
					...
					Logger.d(
							...
							"Delete channel "
									+ _model.getChannels()
											.get(v.getId() - 10000)
											.getDescription());
					showDeleteChannelDialog(_model.getChannels().get(
							v.getId() - 10000));
				}
			});
			...
			btnEdit.setLayoutParams(new LinearLayout.LayoutParams(height,
					height));
			btnEdit.setId(1000 + c.getId());// ID for delete should be
											// 100+channelId
			btnEdit.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					i.putExtra(ActivityChannelConfig.EXTRA_MODEL_ID,
							_model.getId());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case CHANNEL_CONFIG_RETURN:
			...
			try {
				...
				Logger.d(TAG,
						"Received channel from ActivityChannelConfig: channel:"
								+ returnChannel.getDescription() + ", silent: "
								+ returnChannel.getSilent());
	...
	void onServerConnected() {
		...
		// ArrayAdapter<CharSequence> alarmLevelAdapter =
		// ArrayAdapter.createFromResource(this, R.array.alarm_level,
		// android.R.layout.simple_spinner_item );
		// init all model types
		ArrayAdapter<CharSequence> modelTypeAdapter = ArrayAdapter
		...
		String[] modelTypes = getApplicationContext().getResources()
				.getStringArray(R.array.model_types);

		spType.setAdapter(modelTypeAdapter);
		for (int i = 0; i < modelTypes.length; i++) {
			Logger.i(TAG,
					"Comparing " + modelTypes[i] + " to " + _model.getType());
